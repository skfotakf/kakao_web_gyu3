...
public class SignUpDaoImpl implements SignUpDao {
	...
	public int phoneNumberCheck(String phone, String name) {
		...
		try {
			con = pool.getConnection();
			sql = "select count(pm.phone_number), count(um.user_phone) from phonenumber_list_mst pm left outer join user_mst um on(um.user_phone = pm.phone_number) where pm.phone_number = ? and pm.phone_username = ?";
			pstmt = con.prepareStatement(sql);
